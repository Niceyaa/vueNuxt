<template>
    <div ref="detailContainer" class="detail-container">
        ...
        <div class="detail-content-container">
            <div class="detail-content">
                <div class="detail-base-info">
                    ...
                    <div @click="openVipModel" v-else class="right-opt-btn notVip">
                        <img src="~/assets/detail/hesd_vip@2x.png" alt="">
                        <div>会员免费使用</div>
                    </div>-->
                    <div @click="createDocByTemplateId" class="right-opt-btn">
                        <img src="~/assets/detail/head_template@2x.png" alt="">
                        <div>依模板创建标书</div>
                    </div>
                </div>
                <div class="detail-main-content">
                    ...
                    <div :ref="'pic'+idx" class="page-content" v-for="(item,idx) in detailContent.urlList" :key="idx">
                        <el-image
                                lazy
                                :src="item">
        ...
        <!--设为自定义素材或模板-->
        <commonSetDialog router-type="sysTemplate" :type.sync="currentType" :key-id="detailId"
...
<script>
    ...
    export default {
        ...
        components: {commonSetDialog, successShow},
        data() {
            return {
                showFlag: false,
                editUrl: null,
                ...
                relateMaterialData: [],
                relateCurrentPage: 1,
                relateMaterialCurrentPage: 1,
                relatePageSize: 10,
                detailId: null,
                type: 2, //type类型 1系统素材 2系统模板 3用户自定义素材 4用户自定义模板
                detailContent: {},
        ...
        async asyncData(context) {
            try {
                ...
                let relateContent = await context.$axios.post('/smartbiddoc-material/api/template/apiQuerySimilarTemplatePage', relatePrm)

                const relateMaterialPrm = {
                    "industryIdList": ind,
                    "labelIdList": temp,
                    "pageNum": 1,
                    "pageSize": 10
                }
                let relateMaterialContent = await context.$axios.post('/smartbiddoc-material/api/material/apiQuerySimilarMaterialPage', relateMaterialPrm)
                return {
                    ...
                    relateMaterialData: relateMaterialContent.data.result.list
                }
            } catch (error) {
                /* context.error({
                     statusCode: 500
        ...
        methods: {
            ...
            //换一换
            changeMaterialRelateData() {
                this.relateMaterialCurrentPage++
                this.getMaterialRelateData()
            },
            //收藏
            collectDoc() {
                if (!this.$store.state.userInfo.userName) {
                ...
                this.$axios[this.$apiURL.user.collectHandle.method](this.$apiURL.user.collectHandle.url, prm).then(res => {
