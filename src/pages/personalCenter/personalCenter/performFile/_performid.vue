<template>
    <div class="template-or-material">
        <div class="temp-material-header">
            <div class="com-name">业绩附件管理</div>
            <div class="com-opt-wrapper">
                <el-button v-if="deleteList.length>0" type="danger" icon="el-icon-delete" @click="multipleDeleteHandle">删除</el-button>
                <el-button type="primary" plain @click="createFold">新建业绩分类</el-button>
                <el-button v-show="currentRelated" type="primary" plain @click="uploadFile">上传业绩附件
                <!--<el-button v-show="currentRelated" type="primary" plain icon="el-icon-upload" @click="uploadFile">上传业绩附件-->
                </el-button>
            </div>
        </div>
        <div class="temp-material-content">
            ...
            <el-table
                ...
                <el-table-column
                        ...
                        min-width="200px">
                    <template v-slot="{row}">
                        <div v-if="!row.performEncId" class="type-wrapper" @click="getInnerClassifyData(row)">
                            <img src="~/assets/personal2/personal_icon_file@2x.png" alt="">

                            <div class="file-name">{{row.performTypeName}}</div>
                        </div>
                        <div v-else class="type-wrapper">
                            ...
                            <div class="file-name">{{row.performFileName}}</div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
                    ...
                    <template v-slot="{row}">
                        <div style="display: flex;align-items: center;">
                            <common-button v-if="!row.performEncId" @click.native="renameOpen(row)" :icon="'el-icon-edit-outline'"
                            ...
                            <common-button v-if="row.performEncId" @click.native="deleteDocHandle(row)" :icon="'el-icon-delete'"
                        ...
                        <!--<el-dropdown class="opt-dropdown" @command="handleCommand($event,row)">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <el-dropdown-item v-if="!row.performEncId"  command="rename">重命名</el-dropdown-item>
        ...
        <!--新建自定义模板-->
        <el-dialog class="setFodderDialog" width="440px" title="添加业绩附件" :visible.sync="newFodderFlag">
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.performId = this.$route.params.performid
            this.initData()
        },
        methods: {
            initData() {
            ...
            getData() {
                ...
                const prm = qs.stringify({
                    ...
                    "performTypeId": this.currentRelated,
                })
                this.$axios[this.$apiURL.certification.getPerformTypeAndFile.method](this.$apiURL.certification.getPerformTypeAndFile.url, prm).then(res => {
                    ...
                    axios({
                        ...
                        if (res.data.code === 200) {
                            ...
                            const filePrm = {
                                performId: this.performId,
                                "performTypeId": this.currentRelated,
                                "performFileId": this.currentFileId,
                                "performFileName": this.setFodderForm.cusMaterialName
                            }
                            this.$axios[this.$apiURL.certification.uploadPerformFile.method](this.$apiURL.certification.uploadPerformFile.url, filePrm).then(fileRes => {
            ...
            createFold() {
                this.$prompt('请输入文件夹名字', '提示', {
                    ...
                    const prm = {
                        performTypeName: value,
                        pid: this.currentRelated ? this.currentRelated : 0,
                    }
                    this.$axios[this.$apiURL.certification.addPerformanceType.method](this.$apiURL.certification.addPerformanceType.url, prm).then(res => {
                        this.$message({
                            type: 'success',
            ...
            getInnerClassifyData(info) {
                this.currentRelated = info.performTypeId
                this.alreadyExistClassify.push({
                    performTypeId: info.performTypeId,
                    name: info.performTypeName
                })
                this.getData()
            },
            //点击面包屑获取数据
            goToInnerClassifyData(info) {
                if (info.performTypeId === this.currentRelated) return
                let idx1 = 0
                this.alreadyExistClassify.forEach((item, idx) => {
                    if (item.performTypeId === info.performTypeId) {
                        idx1 = idx
                    }
                })
                ...
                switch (command) {
                    ...
                    case 'download':
                        if (info.docType === 'png' || info.docType === 'jpg' || info.docType === 'jpeg') {
                            ...
                            this.previewImgUrl = info.performFileUrl
                            return
                        }
                        location.href = info.performFileUrl
                        break
                }
            },
            //批量删除
            multipleDeleteHandle() {
                this.$confirm('此操作将永久删除该文件夹和文件夹下面的数据, 是否继续?', '提示', {
                    ...
                    this.$axios[this.$apiURL.certification.multiplePerformInfo.method](this.$apiURL.certification.multiplePerformInfo.url, this.deleteList).then(res => {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
            ...
            deleteHandle(info) {
                this.$confirm('此操作将永久删除该文件夹和文件夹下面的数据, 是否继续?', '提示', {
                    ...
                    this.$axios[this.$apiURL.certification.deletePerformanceType.method](this.$apiURL.certification.deletePerformanceType.url, prm).then(res => {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
            ...
            deleteDocHandle(info) {
                this.$confirm('此操作将永久删除该文档信息, 是否继续?', '提示', {
                    ...
                    const prm = qs.stringify({
                        performEncId: info.performEncId
                    })
                    this.$axios[this.$apiURL.certification.deletePerformFile.method](this.$apiURL.certification.deletePerformFile.url, prm).then(res => {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
            ...
            renameOpen(info) {
                this.$prompt('请输入文件夹名字', '提示', {
                    ...
                    cancelButtonText: '取消',
                    inputPattern: '',
                    inputValue: info.performTypeName,
                    inputErrorMessage: '邮箱格式不正确'
                }).then(({value}) => {
                    const prm = {
                        performTypeName: value,
                    ...
                    this.$axios[this.$apiURL.certification.updatePerformanceType.method](this.$apiURL.certification.updatePerformanceType.url, prm).then(res => {
                        this.$message({
                            type: 'success',
                            message: res.data.msg
                        });
                ...
                if (sel.length > 0) {
                    sel.forEach(item => {
                        this.deleteList.push({
                            keyId:item.performEncId?item.performEncId:item.performTypeId,
                            typeName: item.type
                        })
                    })
                }
